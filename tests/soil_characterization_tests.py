...
from system.soil import Soil, Item, SystemRelation, Evidence, generate_soil_uuid, SOIL_UUID_PREFIX
...
class TestItemCreation:
    ...
    def test_item_stored_in_database(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            uuid = soil.create_item(item)

            # Verify item was created
            retrieved = soil.get_item(uuid)
    ...
    def test_item_uuid_without_prefix_is_accepted(self):
        """get_item() should accept UUIDs without 'soil_' prefix."""
        with tempfile.TemporaryDirectory() as tmpdir:
            db_path = Path(tmpdir) / "test.db"
            soil = Soil(db_path)
            soil.init_schema()
            ...
            retrieved = soil.get_item(uuid_without_prefix)
            assert retrieved is not None
...
class TestSystemRelations:
    ...
    def test_replies_to_relation_helper(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            soil.create_item(parent)
            soil.create_item(reply)
...
class TestDatabaseInitialization:
    ...
    def test_context_manager_works(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            soil2 = Soil(db_path)
            retrieved = soil2.get_item(item.uuid)
            assert retrieved is not None
